...
```
내 맥 ──commit──→ (내 맥에만 기록)
      ──push────→ 깃허브 저장소 ──GitHub Pages──→ 주소
                                                    ↑
                                      언니 폰이 앱을 열 때 가지러 온다
```

**주소는 절대 바뀌지 않는다.** 링크를 다시 보낼 일도, 앱을 다시 깔 일도 없다. 커밋만 해서는 아무 일도 일어나지 않고 **푸시를 해야** 주소의 내용이 바뀐다.

폰이 새 버전을 알아채는 방법은 하나다 — 앱을 열 때 **`sw.js` 만 서버에서 받아 글자 단위로 비교**하고, 다르면 앱 파일 18개를 새로 받는다. 그래서 `CACHE` 이름을 안 올리면 **폰은 영원히 옛 버전을 본다.**

**앱을 두 번 열어야 새 화면이 보인다.** 첫 번째로 열 때는 이미 옛 파일로 화면을 그린 *뒤에* 새 파일을 받기 때문이다. 처음 열었을 때 그대로인 건 정상이다.

이 경로에 **예약 데이터는 한 번도 등장하지 않는다.** 오가는 것은 앱 파일뿐이고, 폰에서 서버로 올라가는 것은 없다. 그래서 앱을 갈아엎어도 예약이 남고, 동시에 **폰이 고장 나면 사본이 어디에도 없다.**

> 옛 버전(v12)이 깔린 상태에서 새 버전(v16)으로 갱신되는 것을 실제로 재현해 확인했다 — 새 캐시가 들어오고 옛 캐시가 지워지며, **예약과 손님 이름은 그대로 남았다.**
